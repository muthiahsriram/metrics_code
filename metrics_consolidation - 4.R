...
metrics<-function(metrics_main_icc,inprogress_icc=NULL,user_list_icc,issues_summary_icc,otq="Other_task|Other task",atq="automation",metrics_main_epipe,inprogress_epipe=NULL,user_list_epipe,issues_summary_epipe=NULL)
{
  ...
  columnns_agile_metrics<-ncol(agilemetrics)
  columnns_agile_metrics<-columnns_agile_metrics - 1
  w<-0
  
  ##making data for the whole group
  
  peoplewithissues<-agilemetrics[,c("Testers")]
  ...
  colnames(peoplewithoutissues)<-colnames(agilemetrics)
  
  View(peoplewithoutissues)
  agilemetrics_final<-rbind(agilemetrics,peoplewithoutissues)
  }
...
View(lbls)
pie(issues,labels = percentlabels,col=rainbow(length(lbls)),radius=0.5, main="Pie Chart of Issues")
legend("topright", lbls, cex=0.5, fill=rainbow(length(lbls)))
...
pie(percentlabels,labels = percentlabels1,col=rainbow(length(percentlabels1)),radius = 0.5 ,main="Pie Chart of hours spent",cex = 1.0)
legend("topright", lbls1, cex=0.4, fill=rainbow(length(lbls1)))
